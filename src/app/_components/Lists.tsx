...
export function Lists() {
  ...
  const { data: userListEntries, isLoading: isUserListEntriesLoading} = api.list_entries.get_by_user.useQuery({
  ...
  const handleCreateList = async (name: string) => {
    ...
    //   setNewListName(school_id);
      const result = await createList.mutateAsync({ name,  user_id: userObj!.id});
                ...
                Object.entries(flattened).map(([key, value]) => (
                    <div key={key}>
                        ...
                        {value.schools.length === 0 ? (
                            <div className="text-gray-500 italic">No schools in this list yet</div>
                        ) : (
